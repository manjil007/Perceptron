...
plane_boundary = -X1 + (2 * X2) + 5

plt.figure(figsize=(10, 8))

plt.plot(x1, (x1 - 5)/2, 'k-', label='Hyperplane: -x1 + 2x2 + 5 = 0', linewidth=2)

plt.contourf(X1, X2, plane_boundary, levels=[-1000, 0, 1000], colors=['#FFAAAA', '#AAAAFF'], alpha=0.5)

data = pd.read_csv('data/case2.csv', header=None)
...
plt.xlabel('$x1$')
plt.ylabel('$x2$')
plt.title('Hyperplane: $x_1 - 2x_2 + 5= 0$ (or $x_1 = x_2$)')
